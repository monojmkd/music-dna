  ...
  async function handleShare() {
    ...
    try {
      ...
      if (navigator.share && navigator.canShare) {
        ...
        const file = new File([blob], `music-dna-${primary.id}.png`, {
          type: "image/png",
        });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {canNativeShare
            ? "Opens native share sheet · also saves as PNG"
            : "Saves high-res PNG · Perfect for Stories & posts"}
        </p>
      </div>
...
// ─── Read URL params once at module load — pure, no hooks, no rules ───────────
function getInitialState() {
  const p = new URLSearchParams(window.location.search);
  const code = p.get("code");
  const err = p.get("error");
  // replaceState here is safe — module-level, runs once, not during React render
  if (code || err)
...
export default function App() {
  ...
  // All setState calls live inside .then()/.catch() callbacks — the pattern the
  // linter explicitly approves ("calling setState in a callback when external state changes").
  // No setState is called synchronously in the effect body itself.
  useEffect(() => {
    if (!init.code) return;
    ...
    // init.code is stable (lazy useState, never changes). No other deps needed.
